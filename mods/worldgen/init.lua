...
local min_height_noise
local min_height_data = {}
local max_height_noise
...
local function get_density(x, y, z, wx, wy, wz)
	...
	volume_pos.y = wy
	volume_pos.z = wz
	density = math.max(density, volume_noise:get_3d(volume_pos) + gradient)
...
local function fill_density(emin, emax)
	if not section_count then
		...
		min_height_noise = minetest.get_perlin_map({
			offset = 0,
			scale = 1.0,
		...
		max_height_noise = minetest.get_perlin_map({
			offset = 0,
			scale = 1.0,
	...
	min_height_noise:get_2d_map_flat(noise_pos, min_height_data)
	max_height_noise:get_2d_map_flat(noise_pos, max_height_data)
